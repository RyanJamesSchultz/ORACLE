...
class Weibull(Distribution):
    arg_constraints = {"shape": constraints.positive}

    def __init__(
        self, log_mean: torch.Tensor, shape: torch.Tensor, eps=1e-10, validate_args=None
    ):
        scale = torch.exp( ( torch.lgamma(1 + shape.reciprocal()) - log_mean ) * shape )
