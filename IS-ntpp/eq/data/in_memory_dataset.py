...
class InMemoryDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, sequences: List[SequenceIS]):
        if any(not isinstance(seq, SequenceIS) for seq in sequences):
            raise ValueError("sequences must be a list of eq.data.SequenceIS")
        self.sequences = sequences

    def __getitem__(self, key: int) -> SequenceIS:
    ...
    def load_from_disk(path: Union[str, Path]) -> "InMemoryDataset":
        ...
        sequences = [SequenceIS(**seq) for seq in data]
    ...
    def to(self, device):
        ...
        def to_device(seq: SequenceIS) -> SequenceIS:
    ...
    def get_dataloaderIS(self, batch_size=1, shuffle=False, **kwargs):
        return torch.utils.data.DataLoader(
            self,
            batch_size=batch_size,
            shuffle=shuffle,
            collate_fn=BatchIS.from_list,
            **kwargs,
        )
